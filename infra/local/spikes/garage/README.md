...
Human-run, disposable infrastructure for
[ADR 0003](../../../../docs/decisions/0003-local-s3-and-recovery-boundary.md).
It is not part of `local:up`, CI, or `verify:full`. It does not access PostgreSQL,
RabbitMQ, the persistence mock, or application environment files. No application
adapter or backup command is delivered here.
...
Human dependency gate, from the repository root:
...
Review and commit the lockfile generated by that install with this PR; agents
do not regenerate it. Both new root dev dependencies are pinned to `3.1112.0`.
They are used only by the spike, not gateway production code. Do not merge the
manifest additions without the matching lockfile. M04-F will declare the
...
For example, on an Apple Silicon Mac, reset the disposable spike volumes before
selecting the other architecture. This removes the spike's existing objects:

```bash
npm run local:garage:spike -- reset --confirm-disposable
export M04B_PLATFORM=linux/amd64
npm run local:garage:spike -- up
npm run local:garage:spike -- bootstrap
npm run local:garage:spike -- verify-empty
npm run local:garage:spike -- verify
...
Repeat bootstrap and the reset-proof sequence under that architecture as well.
A missing engine emulator is an unexecuted platform gate, not proof of a Garage
failure. A real image/API failure blocks M04-E and requires provider review.
...
Human verification: commit the spike files, workflow, and reviewed lockfile,
then push this branch and open/update its PR into `main`. Inspect the separate
...
provides native-amd64 evidence, not automatic milestone or license acceptance.
...
The human formats/lints the new files, then runs the repository gate:
...
the working M04 plan. Do not mark M04-B or ADR 0003 accepted until those checks
and the human recovery-boundary/source/license review are confirmed.
...
All runtime checks are pending at implementation handoff. Manifest/source
inspection is not a substitute for these checks.
